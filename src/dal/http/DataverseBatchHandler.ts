...
export class DataverseBatchHandler {
    public static readonly maxRetries = 5;
    public batchSeparator: string = "batch-mgwdev-m365-helpers";
    constructor(protected baseClient: IHttpClient, protected dataverseEnvUri, protected apiPath, protected registeredPromises: Map<string, { resolve, error }>, protected batch: IBatch[], protected retries: number = 0) { }
    ...
    protected requestBatch = async (batchBody: string) => {
        ...
        const response = await this.baseClient.post(`${this.dataverseEnvUri}${this.apiPath}/$batch`, options);
        if (response.ok) {
