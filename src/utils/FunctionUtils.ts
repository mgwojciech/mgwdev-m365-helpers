import { ICacheService, SessionStorageCacheService } from "../services/cache";

const defaultCacheService = new SessionStorageCacheService();
export function useStorage(storageKey: string, cacheService: ICacheService = defaultCacheService) {
            ...
            return new Promise(async (resolve, reject) => {
                try {
                    ...
                    originalMethod.apply(this, args).then((result: any) => {
                        cacheService.set(methodRelatedKey, result);
                        resolve(result);
                    }).catch(reject);
                } catch (err) {
                    reject(err);
