import { IQueryField } from '../../model/IQueryField';
...
export class CamlQueryBuilder implements IQueryBuilder {
  ...
  public withFieldQuery(
    ...
    switch (fieldInfo.comparer) {
      ...
      default:
        newQuery = `<${fieldInfo.comparer}><FieldRef Name='${
          fieldInfo.name
        }' /><Value ${
          fieldInfo.includeTimeValue != null
            ? `IncludeTimeValue='${
                fieldInfo.includeTimeValue ? 'TRUE' : 'FALSE'
              }' `
            : ''
        }Type='${fieldInfo.type}'>${fieldInfo.value}</Value></${
          fieldInfo.comparer
        }>`;
        break;
    }
